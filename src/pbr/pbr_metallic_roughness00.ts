...
export namespace DemoActionPBRMR00 {
    ...
    let _material: BABYLON.PBRMetallicRoughnessMaterial;
    ...
    function initCall(canvas: HTMLCanvasElement) {
        ...
        DataPanel.init([
            ['Metallic', 1.0, (v) => { _material.metallic = v }],
            ['Roughness', 0.0, (v) => { _material.roughness = v }],
        ]);
    }
    function uninitCall() {
    ...
    function fresnel() {
        _material = new BABYLON.PBRMetallicRoughnessMaterial(name, _scene.impl);

        _sphere.material = _material;
        _box.material = _material;

        _material.baseTexture = new BABYLON.Texture('app_scene/scene_res/res/images/Hd3bL4PPMEgg8d1VykdvgF.jpg', _scene.impl);
